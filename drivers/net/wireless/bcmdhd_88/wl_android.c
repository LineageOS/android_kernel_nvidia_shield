...
static int gpio_wlan_pwr = -1;
static int gpio_wlan_rst = -1;
static const char *edp_name;
struct sysedp_consumer *sysedpc = NULL;
...
int wifi_set_power(int on, unsigned long msec)
{
	...
	if (wifi_control_data && wifi_control_data->set_power)
		...
		if (gpio_is_valid(gpio_wlan_pwr))
			gpio_set_value_cansleep(gpio_wlan_pwr, on);
		if (gpio_is_valid(gpio_wlan_rst))
			gpio_set_value_cansleep(gpio_wlan_rst, on);
	}
...
static struct resource *get_wifi_irqres_from_of(struct platform_device *pdev)
{
	...
	if (irq < 0) {
		DHD_ERROR(("%s: get irq fail", __FUNCTION__));
		return NULL;
	}
...
static int wifi_probe(struct platform_device *pdev)
{
	...
	/* get platform data from DT */
	if (pdev->dev.of_node) {
		wifi_irqres = get_wifi_irqres_from_of(pdev);

		gpio_wlan_pwr = of_get_named_gpio(pdev->dev.of_node, "wlan-pwr-gpio", 0);
		gpio_wlan_rst = of_get_named_gpio(pdev->dev.of_node, "wlan-rst-gpio", 0);

		if (of_property_read_string(pdev->dev.of_node, "edp-consumer-name", &edp_name)) {
                        sysedpc = NULL;
                        DHD_ERROR(("%s: property 'edp-consumer-name' missing or invalid\n",
                                                                        __FUNCTION__));
                } else {
                        sysedpc = sysedp_create_consumer("primary-wifi", edp_name);
                }
	} else {
		...
		if (!wifi_ctrl) {
			...
			if (IS_ERR(regulator))
				DHD_ERROR(("%s: wlreg_on regulator is not available", __func__));
			else
		...
		if (wifi_control_data) {
			sysedpc = sysedp_create_consumer("wifi", "wifi");
		}
	}
