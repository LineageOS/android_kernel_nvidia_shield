...
module_param_array(downgraded_usb3, uint, &downgraded_count,
				    ...
				    struct usb_device *udev)
{
	...
	if (to_downgrade) {
		for (i = 0; i < XUSB_SS_PORT_COUNT; i++) {
		...
		if (i < XUSB_SS_PORT_COUNT) {
			...
			spin_lock_irqsave(&xhci->lock, flags);
			portsc = xhci_readl(xhci,
				xhci->usb3_ports[udev->portnum - 1]);
			portsc |= PORT_PE;
			xhci_writel(xhci, portsc,
				xhci->usb3_ports[udev->portnum - 1]);
			...
			list_add_tail(&port_ptr->downgraded_list,
				&hub_downgraded_list);
			portsc = xhci_readl(xhci,
				xhci->usb3_ports[udev_portnum - 1]);
			portsc &= ~(PORT_PLS_MASK | PORT_PE);
			portsc |= (PORT_POWER | XDEV_RXDETECT |
				PORT_LINK_STROBE);
			xhci_writel(xhci, portsc,
...
static const struct hc_driver tegra_plat_xhci_driver = {
	...
	.update_device =	tegra_xhci_update_device,
...
static void downgrade_usb3(struct device *dev, unsigned int downgrade_enabled)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct tegra_xhci_hcd *tegra = platform_get_drvdata(pdev);
	struct usb_device *hdev20 = tegra->xhci->main_hcd->self.root_hub;
	struct usb_hub *hub20 = usb_hub_to_struct_hub(hdev20);
	struct usb_device *hdev30 = tegra->xhci->shared_hcd->self.root_hub;
	...
	if ((downgrade_enabled == 0xffffffff) || (downgrade_enabled == 0)) {
		...
		if (tegra->hc_in_elpg)
			tegra_xhci_host_partition_elpg_exit(tegra);
		spin_lock_irqsave(&tegra->xhci->lock, flags);
		power_up_wait = 0;
		for (port1 = 1; port1 <= hdev30->maxchild; port1++) {
			if (port1 > XUSB_SS_PORT_COUNT)
				break;
			portsc = xhci_readl(tegra->xhci,
				tegra->xhci->usb3_ports[port1 - 1]);
			if (downgrade_enabled == 0xffffffff) {
				if (portsc & PORT_POWER) {
					/* To disconnect */
					portsc |= PORT_PE;
					xhci_writel(tegra->xhci, portsc,
						tegra->xhci->usb3_ports[port1
						- 1]);
					power_up_wait++;
				}
			} else {
		...
		if (power_up_wait) {
			spin_unlock_irqrestore(&tegra->xhci->lock, flags);
			msleep(10);
			spin_lock_irqsave(&tegra->xhci->lock, flags);
		}
		if (downgrade_enabled == 0xffffffff) {
			for (port1 = 1; port1 <= hdev30->maxchild; port1++) {
				...
				if (portsc & PORT_POWER) {
					...
					spin_unlock_irqrestore(
						&tegra->xhci->lock, flags);
					msleep(1);
					spin_lock_irqsave(&tegra->xhci->lock,
						flags);
					portsc = xhci_readl(tegra->xhci,
						tegra->xhci->usb3_ports[port1
						- 1]);
		...
		if (downgrade_enabled == 0) {
			...
			if (!power_up_wait) {
				spin_unlock_irqrestore(&tegra->xhci->lock,
					flags);
				mutex_unlock(&tegra->sync_lock);
				pm_runtime_put(hcd->self.controller);
				return;
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%X", &downgrade_enabled) != 1)
		return -EINVAL;
...
static void downgrade_usb3_remove_file(struct tegra_xhci_hcd *tegra)
{
	struct device *dev = &tegra->pdev->dev;

	if (attr_name(tegra->downgrade_usb3_attr)) {
		device_remove_file(dev, &tegra->downgrade_usb3_attr);
		kzfree(attr_name(tegra->downgrade_usb3_attr));
	}

}

static int downgrade_usb3_create_file(struct tegra_xhci_hcd *tegra)
{
	struct device *dev = &tegra->pdev->dev;
	int err;
	char *downgrade_attr;

	downgrade_attr = kasprintf(GFP_KERNEL, "downgrade_usb3");
	if (!downgrade_attr)
		return -ENOMEM;

	attr_name(tegra->downgrade_usb3_attr) = downgrade_attr;
	tegra->downgrade_usb3_attr.show = downgrade_usb3_show;
	tegra->downgrade_usb3_attr.store = downgrade_usb3_store;
	tegra->downgrade_usb3_attr.attr.mode = (S_IRUGO | S_IWUSR | S_IWGRP);
	sysfs_attr_init(&tegra->downgrade_usb3_attr.attr);

	err = device_create_file(dev, &tegra->downgrade_usb3_attr);
	if (err) {
		kzfree(attr_name(tegra->downgrade_usb3_attr));
		attr_name(tegra->downgrade_usb3_attr) = NULL;
		return err;
	}
...
static int tegra_xhci_probe2(struct tegra_xhci_hcd *tegra)
{
	...
	*((struct xhci_hcd **) xhci->shared_hcd->hcd_priv) = xhci;
	...
	downgrade_usb3_create_file(tegra);
...
static int tegra_xhci_remove(struct platform_device *pdev)
{
	...
	downgrade_usb3_remove_file(tegra);
